...
- **窗口拖拽用原生 `-webkit-app-region: drag`**(`#stage`),**不要**用 IPC 逐帧 `setPosition`(曾导致卡顿 + setPosition 参数转换崩溃)。可交互区域(输入条)必须标 `no-drag`;要点击宠物时在 `#stage` 上叠 `no-drag` 透明层。**窗口缩放(0057 起 win32 用 Electron 原生边缘缩放,0063 修复"四角拖动偶发无响应")**:创建窗口即 `resizable: true` —— Electron 对 `transparent` 窗口强制关闭 thickFrame(WS_THICKFRAME 永不在样式里,所以 0056e 的 `SendMessageW(WM_NCLBUTTONDOWN, HT*)` 注入**永不进入系统 sizing 循环**:Chromium 窗口过程会吞掉该消息,实测所有变体瞬间返回,`native-resize.ts` 已删除),Electron **自带 frameless 边缘命中测试与缩放实现**(边缘按下被系统非客户区命中测试(HTRIGHT)吞掉,renderer 手柄收不到 pointerdown,`#stage` 的 drag 区域也不会抢边缘)。**两个坑(0063)**:1) 原生命中区只有窗口最外约 5px(Chromium `kResizeInsideBoundsSize`),而 renderer 手柄是角 24×24/边 8px(no-drag + resize 光标)—— 手柄圈内 5px 之外的环形区域按下既不是缩放也不是拖动,**完全无反应**("有概率拖不动"= 落点几何彩票)。修复 = **环形区改走 0056 轮询缩放**:win32 手柄照常注册 pointerdown(原生 5px 上的按下被系统吞掉,不会触发),环形区按下 → `pet:resize-start` → 主进程 16ms 锚定对侧边 setBounds 专用循环;原生 5px 保持原生 sizing 循环,两者并存、互不冲突。0056e 时代测得的"轮询弹跳"是 33ms 共享轮询 + backdrop-filter 未禁用时的问题 —— 现在 16ms 专用循环 + `body.pet-resizing` 禁用玻璃 backdrop(0057)后已无此因素,若再出现内容弹跳优先查这条。**勿再尝试"原生回调进 JS"方案**:koffi 回调在 Electron 主进程的 OS 消息泵里被排队延迟执行(非同步),回调里再调 `DefSubclassProc` 已脱离系统 dispatch 上下文,实测 `COMCTL32.dll` ACCESS_VIOLATION 与 `0xC000041D`(STATUS_FATAL_USER_CALLBACK_EXCEPTION)崩溃 —— WM_NCHITTEST 子类(SetWindowSubclass + koffi 回调)不可行,`win32-resize-hit.ts` 已删除;同批教训:`koffi.register(fn, type)` 的 type 必须是 `koffi.pointer(koffi.proto(...))`(裸 proto 抛 "expected \<callback\> \* type"),回调实参里 `uintptr` 以 Number 传入(不是 BigInt),`void*` 是 bigint —— 仅剩的 FFI 是 `win32-key-state.ts` 的 `GetAsyncKeyState`(从 JS 同步调用,与 rounded-window.ts 的 DWM 调用同款,安全)。2) **手势结束不能靠"resize 事件静默"判定** —— 拖拽中途光标停顿/尺寸被 MIN-MAX 夹住时事件同样停发,静默即结束会在手势仍激活时改 DWM 圆角属性 + 同步写盘,打断系统 sizing 循环、窗口停止跟手;修复 = 结束以 `resized` 事件为准,缺失时 250ms 静默兜底**且 `GetAsyncKeyState(VK_LBUTTON)` 确认左键已松开**(仍按下=暂停,只重新武装计时器)。**手势开始/结束信号经 `pet:resize-gesture` 按状态去重推 renderer**(renderer 订阅切 `body.pet-resizing`:禁用 drag 区域 + 仅禁玻璃组件 backdrop-filter 与过渡 —— 透明窗口+实时缩放时 backdrop 采样滞后是"组件跳动/闪烁"的经典来源,0057;背景保持原有半透明,**不要**提亮;renderer 本地 resize/pointer 逻辑保留为兜底);结束(左键松开)时把最终尺寸落盘 `appearance.windowWidth/Height` 并重设圆角(DWM 属性只在真正结束后重设)。MIN/MAX 由 `setMinimumSize/setMaximumSize`(WM_GETMINMAXINFO)自动夹取。**非 win32** 沿用 0056:renderer 8 条透明 no-drag 手柄发 `pet:resize-start/end`,主进程 60Hz 锚定对侧边 setBounds(轮询方案内容会来回弹跳,仅限非 win32 兜底)。
