...
- **窗口拖拽用原生 `-webkit-app-region: drag`**(`#stage`),**不要**用 IPC 逐帧 `setPosition`(曾导致卡顿 + setPosition 参数转换崩溃)。可交互区域(输入条)必须标 `no-drag`;要点击宠物时在 `#stage` 上叠 `no-drag` 透明层。**窗口缩放(0056)**:renderer 铺 8 条透明 `no-drag` 边缘手柄,只在按下/松开时发 `pet:resize-start/end`(edge ∈ n/s/e/w/ne/nw/se/sw);**win32 走原生系统 sizing 循环(`native-resize.ts`:koffi `SendMessageW(hwnd, WM_NCLBUTTONDOWN, HT*)`,阻塞到松开,框架/内容由 DWM 一起移动 —— 程序化 setBounds 缩放会导致内容来回弹跳,轮询方案只能缓解无法根治,0056e)**,MIN/MAX 由 `setMinimumSize/setMaximumSize`(WM_GETMINMAXINFO)在系统循环内夹取,松开后落盘到 `appearance.windowWidth/Height`;非 win32 / 原生未接管(瞬间返回)回落 60Hz 锚定对侧边 setBounds。**缩放期间必须临时把 `#stage` 的 drag 区域改 no-drag(`body.pet-resizing`,0056d)**:否则鼠标移入 drag 区域时 Chromium 会向 OS 发起原生窗口移动(WM_NCLBUTTONDOWN/HTCAPTION),与缩放互相覆盖 → 窗口/宠物快速来回跳动;同时缩放期间冻结视线跟随与拖动反馈(光标在边缘,跟踪会导致头部来回摆)。
