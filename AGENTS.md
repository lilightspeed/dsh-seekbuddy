...
- **Live2D motion 播放四坑(0037 实测)**:
  ...
  4. **loop 动画循环点跳变**:素材曲线首尾不一致(如摸头 `EyeLSmile` 0s=0 / 3.833s=1)时,循环点处表情闪没重来(V2 的 correctEndPoint + loop fade-in 只能平滑无法消除)。**摸头强制非循环**(MOTION_FILES 配置 loop:false),播一遍自然结束自动复位;素材曲线首尾对齐后才适合 loop(淡入已禁用,循环点跳变更要靠素材作者对齐曲线/在 Live2D 里画淡入淡出)。
