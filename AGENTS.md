...
- **Live2D motion 播放四坑(0037 实测)**:
  ...
  2. `CubismMotion.create` **不读 json 的 `Loop` 字段**(create 内赋值被注释),须按配置显式 `setLoop`;素材未写 FadeInTime 时默认 1.0s 淡入(表情渐入太慢像没反应),运行时压到 0.15s(`setFadeInTime`)。`_motionBehavior` 默认已是 V2(不用设)。
  ...
  4. **loop 动画循环点跳变**:素材曲线首尾不一致(如摸头 `EyeLSmile` 0s=0 / 3.833s=1)时,循环点处表情闪没重来(V2 的 correctEndPoint + loop fade-in 只能平滑无法消除)。**摸头强制非循环**(MOTION_FILES 配置 loop:false),播一遍自然结束自动复位;素材曲线首尾对齐后才适合 loop。
  5. **动画切换残留(0037u)**:startMotion 若整体取消上一动画的复位(expressionReset),新动画曲线**不覆盖**的参数会卡在残留值——sad(不驱动 EyeLOpen/EyeLSmile)接管摸头时,摸头的闭眼/微笑残留 → **两个表情叠加**。修复:startMotion 解析新动画 Curves 参数集(`parseMotionParamIds` 缓存于 motionParamIds),复位缩减为"新动画不驱动的参数"继续拉回(`keepUncoveredReset`);expressionReset 存 `{id, index}` 列表。**新增表情素材时留意其曲线驱动参数集**——换着打断测试(摸头↔sad)确认无残留。
  6. **素材未写 FadeOutTime 时 SDK 默认 1.0s 淡出(0038)**:`ACubismMotion.parse` 里 `_fadeOutSeconds` 缺省 1.0(编辑器无此默认)——**短动作(如思考 0.5s)会被全程淡出压扁,曲线到不了终点值**(0038 实测:ParamArmRChange 只到 0.52,待机/思考两只右手各约 50% 半透明)。`startMotion` 已统一 `setFadeOutTime(0)` 禁用(SDK fadeOut 拉向当前值本就不可用,见坑 3);新增动作素材后若出现"动画播不完/值偏小",先查这条。
