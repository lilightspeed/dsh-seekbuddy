...
- **窗口拖拽用原生 `-webkit-app-region: drag`**(`#stage`),**不要**用 IPC 逐帧 `setPosition`(曾导致卡顿 + setPosition 参数转换崩溃)。可交互区域(输入条)必须标 `no-drag`;要点击宠物时在 `#stage` 上叠 `no-drag` 透明层。**窗口缩放(0056)同理**:renderer 铺 8 条透明 `no-drag` 边缘手柄,只在按下/松开时发 `pet:resize-start/end`(edge ∈ n/s/e/w/ne/nw/se/sw),尺寸计算在主进程 33ms 光标轮询里做(锚定对侧边 `setBounds`,夹取 MIN/MAX),**不做逐帧 IPC**;拖拽结束(`resize-end`)把尺寸落盘到 `appearance.windowWidth/Height`,建窗直接读持久化尺寸,重启保持 —— 旧的"窗口缩放"(`appearance.scale`)设置已移除(load 时对旧配置做一次性迁移)。**缩放期间必须临时把 `#stage` 的 drag 区域改 no-drag(`body.pet-resizing`,0056d)**:否则鼠标移入 drag 区域时 Chromium 会向 OS 发起原生窗口移动(WM_NCLBUTTONDOWN/HTCAPTION),与 setBounds 缩放互相覆盖 → 窗口/宠物快速来回跳动;同时缩放期间冻结视线跟随与拖动反馈(光标在边缘,跟踪会导致头部来回摆)。
