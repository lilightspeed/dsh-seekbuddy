...
- **窗口拖拽用原生 `-webkit-app-region: drag`**(`#stage`),**不要**用 IPC 逐帧 `setPosition`(曾导致卡顿 + setPosition 参数转换崩溃)。可交互区域(输入条)必须标 `no-drag`;要点击宠物时在 `#stage` 上叠 `no-drag` 透明层。**窗口缩放(0057 起 win32 用 Electron 原生边缘缩放)**:创建窗口即 `resizable: true` —— Electron 对 `transparent` 窗口强制关闭 thickFrame(WS_THICKFRAME 永不在样式里,所以 0056e 的 `SendMessageW(WM_NCLBUTTONDOWN, HT*)` 注入**永不进入系统 sizing 循环**:Chromium 窗口过程会吞掉该消息,实测所有变体瞬间返回,`native-resize.ts` 已删除),但 Electron **自带 frameless 边缘命中测试与缩放实现**(实测生效:边缘按下被系统非客户区命中测试(HTRIGHT)吞掉,renderer 手柄收不到 pointerdown,`#stage` 的 drag 区域也不会抢边缘 —— 手柄在 win32 上只剩 no-drag 屏蔽+光标样式,事件死代码);MIN/MAX 由 `setMinimumSize/setMaximumSize`(WM_GETMINMAXINFO)自动夹取。**手势开始/结束信号 = Electron 的 `will-resize`(拖拽中高频)/`resized`(结束)事件**(仅手动缩放触发,setBounds 不触发):主进程按状态变化去重后经 `pet:resize-gesture` 推 renderer 切 `body.pet-resizing`(禁用 drag 区域 + 仅禁玻璃组件 backdrop-filter 与过渡 —— 透明窗口+实时缩放时 backdrop 采样滞后是"组件跳动/闪烁"的经典来源,0057;背景保持原有半透明,**不要**提亮);`resized` 时把最终尺寸落盘 `appearance.windowWidth/Height`(resized 偶发缺失有 400ms 静默兜底)。**非 win32** 沿用 0056:renderer 8 条透明 no-drag 手柄发 `pet:resize-start/end`,主进程 60Hz 锚定对侧边 setBounds(轮询方案内容会来回弹跳,仅限非 win32 兜底)。
